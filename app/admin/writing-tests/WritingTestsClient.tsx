...
interface WritingTestsClientProps {
  initialWritingTests: WritingTest[]
  error: string
}

export default function WritingTestsClient({ initialWritingTests, error: initialError }: WritingTestsClientProps) {
  const router = useRouter()
  const [writingTests, setWritingTests] = useState<WritingTest[]>(initialWritingTests)
  ...
  const handleDelete = async (testId: string, title: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/writing-tests/${testId}`, {
        method: 'DELETE'
      })
    ...
    <div className="space-y-6">
      ...
      <div className="md:flex md:items-center md:justify-between">
        ...
        <div className="mt-4 flex md:mt-0 md:ml-4">
          <Link
            href="/admin/writing-tests/create"
                  ...
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/admin/writing-tests/${test.id}`}
                        ...
                        <Link
                          href={`/admin/writing-tests/${test.id}/edit`}
                          className="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-sm font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                        >
                          <svg className="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
