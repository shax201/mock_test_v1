...
interface ReadingTestsClientProps {
  initialReadingTests: ReadingTest[]
  error: string
}

export default function ReadingTestsClient({ initialReadingTests, error: initialError }: ReadingTestsClientProps) {
  const router = useRouter()
  const [readingTests, setReadingTests] = useState<ReadingTest[]>(initialReadingTests)
  ...
  const handleDelete = async (testId: string, title: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/reading-tests/${testId}`, {
        method: 'DELETE'
      })

      if (response.ok) {
        // Remove from local state
        setReadingTests(readingTests.filter(test => test.id !== testId))
    ...
    <div className="space-y-6">
      ...
      <div className="md:flex md:items-center md:justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage IELTS reading tests, passages, and questions.
          </p>
        </div>
        <div className="mt-4 flex md:mt-0 md:ml-4">
          <Link
            href="/admin/reading-tests/create"
      ...
      {readingTests.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No reading tests</h3>
          <p className="mt-1 text-sm text-gray-500">Get started by creating a new reading test.</p>
          <div className="mt-6">
            <Link
              href="/admin/reading-tests/create"
                  ...
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/admin/reading-tests/${test.id}`}
                        ...
                        <Link
                          href={`/admin/reading-tests/${test.id}/edit`}
                          className="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-sm font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                        >
                          <svg className="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
