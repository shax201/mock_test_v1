...
export default function InstructorReadingTestsPage() {
  ...
  const fetchReadingTests = async () => {
    try {
      const response = await fetch('/api/instructor/reading-tests')
  ...
  const handleDelete = async (testId: string, title: string) => {
    ...
    try {
      const response = await fetch(`/api/instructor/reading-tests/${testId}`, {
        method: 'DELETE'
      })
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        alert(data.error || 'Failed to delete reading test')
      }
    } catch (error) {
      ...
      <div className="md:flex md:items-center md:justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Create and manage IELTS reading tests, passages, and questions.
          </p>
        </div>
        <div className="mt-4 flex md:mt-0 md:ml-4">
          <Link
            href="/instructor/reading-tests/create"
                ...
                <li key={test.id}>
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/instructor/reading-tests/${test.id}/edit`}
