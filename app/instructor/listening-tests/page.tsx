...
export default function ListeningTestsPage() {
  ...
  const fetchListeningTests = async () => {
    try {
      const response = await fetch('/api/instructor/listening-tests')
  ...
  const handleDelete = async (testId: string, title: string) => {
    ...
    try {
      const response = await fetch(`/api/instructor/listening-tests/${testId}`, {
        method: 'DELETE'
      })
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        alert(data.error || 'Failed to delete listening test')
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="md:flex md:items-center md:justify-between">
        ...
        <div className="mt-4 flex md:mt-0 md:ml-4">
          <Link
            href="/instructor/listening-tests/create"
                ...
                <li key={test.id}>
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/instructor/listening-tests/${test.id}/edit`}
