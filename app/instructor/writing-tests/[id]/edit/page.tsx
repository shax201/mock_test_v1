...
export default function EditWritingTestPage() {
  ...
  const fetchTestData = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/instructor/writing-tests/${params.id}`)
      const data = await response.json()

      if (response.ok) {
        setTestData(data.writingTest)
      } else {
        setError(data.error || 'Failed to fetch writing test')
      }
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-gray-500">Loading writing test...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="md:flex md:items-center md:justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Update the writing test details, passages, and questions.
          </p>
        </div>
