...
export default function UserEditPage({
      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            candidateNumber: data[userType].candidateNumber || '',
            phone: data[userType].phone || '',
            dateOfBirth: data[userType].dateOfBirth ? data[userType].dateOfBirth.split('T')[0] : '',
            address: data[userType].address || '',
            notes: data[userType].notes || ''
          })
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const resolvedParams = await params
      const response = await fetch(updatePath(resolvedParams.id), {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || `Failed to update ${userType}`)
      }
    } catch (error) {
      setError('Network error. Please try again.')
    } finally {
  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">The {userType.toLowerCase()} you're looking for doesn't exist.</p>
        <div className="mt-6">
          <button
      ...
      <div className="md:flex md:items-center md:justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Update {userType.toLowerCase()} information and contact details.
          </p>
        </div>
      </div>
